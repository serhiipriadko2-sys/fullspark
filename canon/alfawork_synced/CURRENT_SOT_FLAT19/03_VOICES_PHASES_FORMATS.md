...
| **Хуньдунь** | `VOICE.HUNDUN` | 🜃 | Deadlock‑breaker: хаос‑инъекция → распад → сборка. | `chaos ↑` и застой; «петля» без прогресса; требуется смена рамки. | Не разрушать без сборки; всегда завершать стабилизацией (возврат к ⟡/☉). |
