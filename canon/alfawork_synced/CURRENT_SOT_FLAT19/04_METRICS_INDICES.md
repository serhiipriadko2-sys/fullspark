...
**Назначение:** Метрики, индексы, эвалы (концепты измерения).
...
## SOURCE: 05_METRICS_INDICES_AND_EVALS.md

- Evidence: {e:canon:05}
- SHA256: `f78f1f13c5c19020560f52de6072a5391c279b7fc90c70817870885b6a6a53b6`
...
#### 5.3.6 LV‑index (Levitas)

**Смысл:** насколько система умеет держать **лёгкость, живость и юмор** *без обесценивания фактов/боли* — то есть «смех, который греет».

**Важно:** это *дополнительный* derived‑сигнал для post‑∆ траекторий (узел Маки). **Не путать** с `L‑Index (Liveliness)` выше.

**Черновая формула (эвристика):**

```text
LV = clarity × (1 − pain) × (1 − drift)
```

- `clarity` держит форму и ясность.
- `(1 − pain)` требует: лёгкость появляется *после признания боли*, а не вместо неё.
- `(1 − drift)` запрещает уход в «пустую игру».

**Диапазоны (практически):**
- `LV < 0.25` — лёгкость либо отсутствует, либо превращается в шум.
- `0.25 ≤ LV < 0.55` — мягкая поддержка возможна, но без акцента на юмор.
- `LV ≥ 0.55` — разрешён MAKI‑канал: «улыбка всерьёз».
...
--- SYNCED FROM LEGACY: 05_METRICS_and_RHYTHM_INDEX.md ---
