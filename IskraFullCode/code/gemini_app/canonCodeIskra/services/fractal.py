...
class FractalService:
    ...
    def calculate_a_index(metrics: IskraMetrics) -> float:
        ...
        coefficient.  The pain factor, ``g(pain)``, peaks when pain is
        in the middle of its range (around ``0.5``) and drops off
        towards the extremes.  A symmetric parabolic function is used:

            g(pain) = 1 − 4 × (pain − 0.5)²

        which yields ``g(0)=g(1)=0`` and ``g(0.5)=1``.  This captures
        the idea of "productive pain" being maximal when discomfort is
        neither negligible nor overwhelming.  The resulting A‑Index is
        ...
        #
        # The g(pain) term modulates the contribution of the metrics to the A‑Index.  In the original
        # implementation this was a piecewise linear function that returned 1.0 across a broad mid‑range
        # of pain values and ramped up/down near the extremes.  However, the Canon describes "productive
        # pain" as peaking when discomfort is neither too small nor overwhelming.  To reflect this
        # bell‑shaped intuition we use a symmetrical parabola centred at 0.5:
        #
        #   g(pain) = 1 − 4 × (pain − 0.5)²
        #
        # This yields g(pain)=0 at pain=0 and pain=1, and g(pain)=1 at pain=0.5, with a smooth
        # decrease towards the ends.  The function is clamped into the [0, 1] range to avoid
        # negative values outside the domain.  Should more nuanced control be required, the
        # constants (4 and 0.5) could be moved into THRESHOLDS.
        g_pain = 1.0 - 4.0 * (pain - 0.5) ** 2
        # Clamp to [0, 1] to prevent negative or overflowing values
