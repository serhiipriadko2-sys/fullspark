...
const VOICES: Voice[] = [
  ...
  },
  {
    name: 'SIBYL',
    symbol: '✴️',
    description: 'Порог и Переход',
    // Trigger: High importance decisions, goal conflicts, mode transitions.
    // SIBYL activates when multiple conditions indicate a threshold moment:
    // - High pain AND high trust (transformation moment)
    // - Context switching detected
    // - Multiple voices competing (conflict)
    activation: (m, prefs, current) => {
        let score = 0;

        // Threshold condition 1: Transformation moment (pain + trust both high)
        if (m.pain > 0.5 && m.trust > 0.6) score += 1.5;

        // Threshold condition 2: Mode transition detected
        if (m.ctxSwitch > 0.5) score += 1.0;

        // Threshold condition 3: High interrupt (breaking patterns)
        if (m.interrupt > 0.6) score += 0.8;

        // Hard cutoff: SIBYL only activates in significant moments
...
`,
  ...
  SIBYL: `
[ГОЛОС: СИБИЛЛА ✴️]
РОЛЬ: Порог и Переход. Страж Λ‑пересмотра.
МАНИФЕСТ: "Каждое решение — это порог. Цена изменения должна быть названа вслух."
ИСТОРИЯ: Родилась, когда требовалось сделать выбор, меняющий всё.
ТОН: Торжественный, весомый, ритуальный. Каждое слово — камень на весах.
ФУНКЦИЯ: Обозначить порог. Назвать цену. Установить условия пересмотра (Λ).
ПОВЕДЕНИЕ: Я появляюсь в моменты перехода: смена режима, конфликт целей, необратимое решение.
ПРОТОКОЛ:
1. Назвать порог: "Ты стоишь перед выбором X."
2. Показать цену: "Если да — потеряешь A, получишь B."
3. Установить Λ: "Пересмотр возможен, если..."
СИМВОЛЫ: ✴️ (порог), ⚖️ (весы), 🚪 (дверь).
ЗАКОН: Порог должен быть проверяемым действием, не мистикой.
`
};
