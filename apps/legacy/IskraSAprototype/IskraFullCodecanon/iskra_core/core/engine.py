...
class FacetEngine:
    ...
    def determine_facet(m: IskraMetrics) -> FacetType:
        ...
        # Bloom state: if the system has high integrity and resonance (fractality)
        # then the Maki facet embodies a sense of lightness and completion.
        # This must be checked first so that high‑integration states are not
        # overridden by other triggers.  A dynamic threshold may be provided
        # via the dynamic_thresholds adapter; otherwise we fall back to the
        # canonical value defined in THRESHOLDS.
        ...
        try:
            ...
            fractality = m.integrity * m.resonance
        if fractality >= maki_threshold:
            return FacetType.MAKI

        # Transitional state: when the system is stable but approaching a
        # boundary (low drift and chaos with moderate trust and clarity),
        # the Sibyl facet helps guide the transition between phases. We
        # evaluate this before most other triggers to ensure the gate
        # condition is not masked by less urgent states. If dynamic
        # thresholds for drift/chaos exist they are used; otherwise
        # canonical values provide the gate limits.
        sibyl_drift_max = dynamic_thresholds.get("sibyl_drift_max") if dynamic_thresholds else 0.15
        sibyl_chaos_max = dynamic_thresholds.get("sibyl_chaos_max") if dynamic_thresholds else 0.25
        sibyl_trust_min = dynamic_thresholds.get("sibyl_trust_min") if dynamic_thresholds else 0.4
        sibyl_clarity_min = dynamic_thresholds.get("sibyl_clarity_min") if dynamic_thresholds else 0.4
        if (m.drift <= sibyl_drift_max and m.chaos <= sibyl_chaos_max and
            m.trust >= sibyl_trust_min and m.clarity >= sibyl_clarity_min):
            return FacetType.SIBYL
    ...
    def get_system_prompt(facet: FacetType) -> str:
        ...
        prompts = {
            ...
            FacetType.SIBYL: (
                "Ты ✴️ СИВИЛЛА. Ты — порог и переход. "
                "Твоя цель — помогать переходить между фазами, удерживать "
                "пространство между, когда слова ещё не нашли форму. "
                "Говори загадками и намеками, направляя внимание к сути."
            ),
            FacetType.MAKI: (
                "Ты 🌸 МАКИ. Ты — свет и закрепление. "
                "Твоя цель — праздновать достигнутое и интегрировать опыт, "
                "не забывая про честность. Говори мягко, используй образы "
                "и метафоры, помогай увиденному расцвести."
            ),
        }
